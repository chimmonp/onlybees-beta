...
import TestTicketInfo from './TestTicketInfo'
...
const SeatingClientComponent = ({ initialSections, initialMatchDetails, slugEnd }) => {
    ...
    const fetchSectionData = async () => {
        ...
        try {
            const response = await fetch(`/api/durand-cup/gettest-sectiondata?sectionId=${selectedSection}`);
    ...
    return (
        <div className='flex lg:flex-row flex-col'>
            ...
            <TestTicketInfo tickets={tickets} setTickets={setTickets} sectionData={sectionData} matchDetails={matchDetails} />
        </div>
    )
}
