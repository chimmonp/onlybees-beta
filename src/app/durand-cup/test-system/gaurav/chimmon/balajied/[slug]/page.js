import TestSeatingClientComponent from '@/app/durand-cup/components/TestSeatingClientComponent';
import { cache } from 'react';

const fetchAllSections = cache(async () => {
    try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL}/api/durand-cup/allsections-test`, {
            next: { revalidate: 0 }
        });
