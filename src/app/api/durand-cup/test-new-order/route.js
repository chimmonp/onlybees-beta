...
export async function POST(req, res) {
    try {
        ...
        if (order.status !== "SUCCESS") {
            ...
            const section = await TestSection.findById(order.section);
            // console.log(section)
            if (!section) {
            ...
            return new Response(JSON.stringify({ success: true, redirectUrl: `${process.env.NEXT_PUBLIC_SITE_URL}/durand-cup/success/` }), { status: 200 });
        }
            ...
            return new Response(JSON.stringify({ success: false, error: 'Payment Pending', redirectUrl: `${process.env.NEXT_PUBLIC_SITE_URL}/durand-cup/payment-pending/` }), { status: 200 });
        }
            ...
            return new Response(JSON.stringify({ success: false, error: 'Payment Failed', redirectUrl: `${process.env.NEXT_PUBLIC_SITE_URL}/durand-cup/failed` }), { status: 200 });
        }
